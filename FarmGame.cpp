   ...
   /* Farm farm = Farm();*/
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            { // обрабатывает позицию мыши в случае изменения размера окна
                relevantWindowSize = sf::Vector2f(event.size.width, event.size.height);
            }
            if (event.type == sf::Event::MouseButtonPressed)
            { // записывает положение мыши при нажатии кнопки
            ...
            { // обрабатывает открытие и закрытие инвентаря
                showInv *= -1;
            }
            if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::B) {
                // управляет открытием и закрытием магазина
                showShop *= -1;
            }
            //shop options
